...
path = '../ecommerce-dataset/'

def load_events(path):
    print('loading raw data from events.csv ...')
    behaviordata = pd.read_csv(path + 'events.csv')
    ...
    for idx in range(behaviordata.shape[0]):
        if behaviordata['event'][idx] == 'view':
...
def build_newdict(user2item, n_minitems):
    ...
    index_dict = dict()
    print('indexing ...')
    print('n_user: {}'.format(len(userlist)))
    print('n_item: {}'.format(len(itemlist)))
    i = 0
